...
```{r}
# Calculate the interpretation of coefficient

```

```{r}
# Define the custom functions for modelsummary to work
tidy.brmsfit <- function(x, ...) {
  s <- as.data.frame(posterior_summary(x, probs = c(0.025, 0.975)))
  ret <- data.frame(
    term = rownames(s),
    estimate = s[, "Estimate"],
    conf.low = s[, "Q2.5"],
...
glance.brmsfit <- function(x, ...) {
  looic <- tryCatch(loo(x)$estimates["elpd_loo", "Estimate"], error = function(e) NA)
  waic <- tryCatch(waic(x)$estimates["elpd_waic", "Estimate"], error = function(e) NA)
  r2 <- tryCatch(brms::bayes_R2(x)[1], error = function(e) NA)
  data.frame(
    LOOIC = -2 * looic,
    WAIC = -2 * waic,
    R2 = r2
