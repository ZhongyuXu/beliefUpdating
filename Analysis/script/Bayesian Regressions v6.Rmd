...
load_all_brms_models <- function(folder = "models") {
  ...
  if (!dir.exists(folder)) stop(paste("Folder not found:", folder))
  ...
  # Load all models using brm(file = ...)
  models <- lapply(model_files, function(f) {
    # message("Loading model from: ", f)
    brm(file = sub("\\.rds$", "", f))  # remove .rds extension
  })

  # Assign model names based on file names (no extension)
  names(models) <- sub("\\.rds$", "", basename(model_files))
...
```{r}
### Random Intercept Only
r11_diffuse <- run_beta_regression(DQ_C_o ~ urns_trans + colours_trans + seqBall_trans  + (1 | participantID), makePrior(), "r11_diffuse")

r11c_diffuse <- run_beta_regression(DQ_C_o ~ colours_trans + seqBall_trans  + (1 | participantID), makePrior(), "r11c_diffuse")
...
```{r}
### Random Intercept and slope
r11_diffuse_rs <- run_beta_regression(DQ_C_o ~ urns_trans + colours_trans + seqBall_trans  + (1 +  urns_trans + colours_trans + seqBall_trans| participantID), makePrior(randomSlope = T), "r11_diffuse_rs")

r11c_diffuse_rs <- run_beta_regression(DQ_C_o ~ colours_trans + seqBall_trans  + (1 + colours_trans + seqBall_trans| participantID), makePrior(randomSlope = T), "r11c_diffuse_rs")
...
```{r}
print(summary(models$r12c2_diffuse))
```
...
compare_models <- function(models) {
  ...
  check_convergence <- function(model) {
    ...
    ess_vals <- c(summ$fixed$ESS_bulk, summ$random$ESS_bulk)
    list(
      rhat_ok = all(rhat_vals <= 1.01, na.rm = TRUE),
      ess_ok = all(ess_vals > 4000, na.rm = TRUE),
      all_ok = all(rhat_vals <= 1.01, na.rm = TRUE) && all(ess_vals > 4000, na.rm = TRUE)
    )
  }
  ...
  for (base in base_names) {
    ...
    if (conv_slope$all_ok) {
      bf <- calculate_bayes_factor(model_slope, model_int)
      if (bf > 3) {
        selected_models[[base]] <- model_slope
...
unique(gsub("_rs$", "", names(models)))[1]
models[unique(gsub("_rs$", "", names(models)))[1]]
```
