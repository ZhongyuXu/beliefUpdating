  ...
  if (stat == "hist") {
    ...
    p <- ggplot() +
      ...
      theme(
        ...
        legend.position = c(0.05, 0.9),  # ← Top-left corner (tweak if needed)
...
plotPosteriorPredictors <- function(brms_model, width, height) {
  ...
  # Shared x-axis limits from 1st–99th percentiles
...
🔹 1. Prior Model Probability: The prior model probability is your belief in a model being true before you see any data.

🔹 2. Bayes Factor (BF): When data D are observed, the Bayes factor quantifies how much more (or less) likely those data are under one model than the other

```         
•   If $BF_{10} > 1$: Data favor the numerator model $M_1$.

•   If $BF_{10} < 1$: Data favor the denominator model $M_0$.
```

🔹 3. Posterior Model Probability: After seeing the data, your belief in each model is updated via Bayes' theorem.

🔹 4. What the Function's Plot Shows
...
```         
•   When $BF_{10} = 1$: the curve is a straight 45° line (posterior = prior → no learning).

•   When $BF_{10} > 1$: the curve bows upward (data support $M_1$; posterior \> prior).

•   When $BF_{10} < 1$: the curve bows downward (data support $M_0$; posterior \< prior).
```
...
```         
•   $BF_{10} = 10$ → data are 10× more likely under $M_1$

•   Prior belief $P(M_1) = 0.20$
```
...
🔹 6. Example Interpretation given by the author
...
r1_diffuse <- winning_models$r1_diffuse
r2_diffuse <- winning_models$r2_diffuse
...
if (BF_21[1] > 3) {
  print("Selected Model(Diffuse Prior) Support H1-B")
} else {
  print("Selected Model(Diffuse Prior) Does not Support H1-B")
}
...
r3_diffuse <- winning_models$r3_diffuse
r4_diffuse <- winning_models$r4_diffuse
...
if (BF_43[1] > 3) {
  print("Selected Model(Diffuse Prior) Support H2-C")
} else {
  print("Selected Model(Diffuse Prior) Does not Support H2-C")
}
```
...
r5_diffuse <- winning_models$r5_diffuse
r6_diffuse <- winning_models$r6_diffuse
...
if (BF_65[1] > 3) {
  print("Selected Model(Diffuse Prior) Support H4-A")
} else {
  print("Selected Model(Diffuse Prior) Does not Support H4-A")
}
```
...
r7_diffuse <- winning_models$r7_diffuse
r7c_diffuse <- winning_models$r7c_diffuse
...
if (BF_77c[1] > 3) {
  print("Selected Model(Diffuse Prior) Support H1")
} else {
  print("Selected Model(Diffuse Prior) Does not Support H1")
}
```
...
r8_diffuse <- winning_models$r8_diffuse_rs
r8c_diffuse <- winning_models$r8c_diffuse
...
if (BF_88c[1] > 3) {
  print("Selected Model(Diffuse Prior) Support H3")
} else {
  print("Selected Model(Diffuse Prior) Does not Support H3")
}
```
...
r9_diffuse <- winning_models$r9_diffuse
r9c_diffuse <- winning_models$r9c_diffuse
...
if (BF_99c[1] > 3) {
  print("Selected Model(Diffuse Prior) Support H2")
} else {
  print("Selected Model(Diffuse Prior) Does not Support H2")
}
```
...
r10_diffuse <- winning_models$r10_diffuse_rs
r10c_diffuse <- winning_models$r10c_diffuse
...
if (BF_1010c[1] > 3) {
  print("Selected Model(Diffuse Prior) Support H4")
} else {
  print("Selected Model(Diffuse Prior) Does not Support H4")
}
```
...
r11_diffuse <- winning_models$r11_diffuse
r11c_diffuse <- winning_models$r11c_diffuse
...
if (BF_1111c[1] > 3) {
  print("Selected Model(Diffuse Prior) Support H1A")
} else {
  print("Selected Model(Diffuse Prior) Does not Support H1A")
}
```
...
r12_diffuse <- winning_models$r12_diffuse
r12c1_diffuse <- winning_models$r12c1_diffuse_rs
r12c2_diffuse <- winning_models$r12c2_diffuse_rs
...
if (BF_1212c1[1] > 3) {
  print("Selected Model(Diffuse Prior) Support H2A")
} else {
  print("Selected Model(Diffuse Prior) Does not Support H2A")
}

print(paste0("BF is ", BF_1212c2))
if (BF_1212c2[1] > 3) {
  print("Selected Model(Diffuse Prior) Support H2B")
} else {
  print("Selected Model(Diffuse Prior) Does not Support H2B")
}
```
...
r13_diffuse <- winning_models$r13_diffuse
r13c1_diffuse <- winning_models$r13c1_diffuse
r13c2_diffuse <- winning_models$r13c2_diffuse_rs
...
if (BF_1313c1[1] > 3) {
  print("Selected Model(Diffuse Prior) Support H3A")
} else {
  print("Selected Model(Diffuse Prior) Does not Support H3A")
}

print(paste0("BF is ", BF_1313c2))
if (BF_1313c2[1] > 3) {
  print("Selected Model(Diffuse Prior) Support H3B")
} else {
  print("Selected Model(Diffuse Prior) Does not Support H3B")
}
```
...
summary(winning_models$r11_diffuse)
```
